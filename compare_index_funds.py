...
def nifty_nasdaq_history(nfty, nsdq):
    ...
    """
    nasdaq = {}
    converted_date = [datetime.strptime(mydate, '%Y-%m-%d').strftime('%y-%m') for mydate in nsdq.Date]
    nasdaq['date'] = np.array([str(mydate) for mydate in converted_date])
    nasdaq['value'] = np.array(nsdq.Close)
    nasdaq['percentage'] = ((nasdaq['value'] - nasdaq['value'][0]) / nasdaq['value'][0]) * 100
    ...
    nifty = {}
    converted_date = [datetime.strptime(mydate, '%b %y').strftime('%y-%m') for mydate in nfty.Date]
    nifty['date'] = np.array([str(mydate) for mydate in converted_date])[::-1]
    nifty['value'] = np.array(nfty.Price)[::-1]
    nifty['value'] = np.array([float(value.replace(',','')) for value in nifty['value']])
    nifty['percentage'] = ((nifty['value'] - nifty['value'][0]) / nifty['value'][0]) * 100
